...
console.log(\`Position constraint: \${globalMod.getPositionConstraint()}\`);
...
const globalMod = seq.globalMods[0];
console.log(\`Modification: \${globalMod.modValue.primaryValue}\`);
console.log(\`Limit per position: \${globalMod.getLimitPerPosition()}\`);
...
console.log(\`CoMKP (Colocalize with Known): \${seq1.globalMods[0].getColocalizeKnown()}\`);
console.log(\`CoMUP (Colocalize with Unknown): \${seq2.globalMods[0].getColocalizeUnknown()}\`);`,
...
console.log(\`Position constraint: \${globalMod.getPositionConstraint()}\`);
console.log(\`Limit per position: \${globalMod.getLimitPerPosition()}\`);
console.log(\`CoMKP: \${globalMod.getColocalizeKnown()}\`);
    ...
    try {
      ...
      const globalModsInfo = this.parsedSequence.globalMods.map((mod: any, index: number) => {
        return {
          ...
          positionConstraint: mod.getPositionConstraint() || null,
          limitPerPosition: mod.getLimitPerPosition() || null,
          colocalizeKnown: mod.getColocalizeKnown() || false,
          colocalizeUnknown: mod.getColocalizeUnknown() || false
        };
      });
